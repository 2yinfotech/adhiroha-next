...
// Paths that still only exist on the old PHP site, so an article linking to one
// has to leave this app. Everything else that used to be listed here —
// apply-for-teacher-in-rishikesh, student-admission-panel and
// yoga-teacher-training-course-rishikesh-india — is a real route now, and
// keeping them here sent readers off-site through a redirect instead of to the
// local page.
  ...
  out = out.replace(
    ...
    (full, pre, href, post, inner) => {
      ...
      if (liveHosted.has(path)) {
        return `<a${pre}href="https://www.adhiroha.com/${path}"${post} target="_blank" rel="noopener noreferrer">${inner}</a>`;
      }
