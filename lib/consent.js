...
/**
 * Is an explicit yes/no required for a visitor in this country?
 *
 * The same list the Consent Mode defaults use, asked as a question. Inside the
 * EEA, the UK and Switzerland consent must be given before anything is set, so
 * the banner has to be shown. Everywhere else the default is already "granted"
 * — the banner used to be shown there anyway, which meant inviting people to
 * refuse something they had not been asked to allow in the first place.
 *
 * Anything unrecognised is treated as requiring consent. A country we could not
 * work out must land on the side that asks.
 */
export function consentRequiredFor(countryCode) {
  const code = String(countryCode || "").trim().toUpperCase();
  if (code.length !== 2) return true;
  // Cloudflare's own "I could not tell" values: XX when the IP resolves to
  // nowhere, T1 for traffic arriving over Tor. Both are two letters and neither
  // is a country, so without this they would fall through as "not the EEA" and
  // the visitor would never be asked. Not knowing is a reason to ask.
  if (code === "XX" || code === "T1" || code === "ZZ") return true;
  return CONSENT_REGIONS.includes(code);
}
