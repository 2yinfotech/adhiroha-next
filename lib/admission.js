...
//
// Env: DB_HOST/DB_PORT/DB_USER/DB_PASSWORD/DB_NAME (REG_* accepted too, see
// lib/db-config.js), SMTP_*.
...
async function getPool() {
  ...
  _pool = await openPool("admission", siteDbConfig());
  return _pool;
}
...
/* ============================================================
   ...
   ============================================================ */
// December and January put men in triple rooms only. Kept as a named constant
// so the rule is visible rather than buried in a condition.
const MALE_TRIPLE_ONLY_MONTHS = ["December", "January"];
...
export async function getRoomAvailability(month, gender) {
  ...
  if (!MONTHS.includes(month) || !["male", "female"].includes(gender)) {
    return { availableRooms: [], doubleSharingBooked: true, tripleSharingBooked: true, unavailableReason: { double: "", triple: "" } };
  }
  let rooms;
  if (USE_SQL_FILE) {
    ...
    const pool = await getPool();
    // rooms_name has one column per month, but only up to the point the DB was
    // last extended (currently through October). A batch in a month with no
    // column yet (e.g. November/December) simply has no bookings, so every room
    // is available — query the column only when it exists, otherwise treat the
    // month as empty rather than letting "Unknown column" crash to both-booked.
    const [cols] = await pool.query(
      "SELECT COLUMN_NAME FROM information_schema.COLUMNS " +
        "WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'rooms_name' AND COLUMN_NAME = ?",
      [month]
    );
    if (cols.length) {
  ...
  // Men are housed differently in the winter batches: over December and January
  // they go into triple rooms and double sharing is not offered at all. Outside
  // those two months the older rule applies, where a triple only opens to a man
  // once another man is already in one.
  // Badge text for an option that is switched off. Only set when the option is
  // closed by policy: "Fully booked" is the default and would be a lie here,
  // since the double rooms in these months are empty, just not on offer.
  let unavailableReason = { double: "", triple: "" };
  if (gender === "male") {
    if (MALE_TRIPLE_ONLY_MONTHS.includes(month)) {
      doubleFull = true;
      unavailableReason.double = "Not available";
      available = available.filter((r) => TRIPLE_ROOMS.includes(r));
      // Genuine capacity still decides: if every triple is taken, it is taken.
      tripleFull = !available.length;
    } else if (!TRIPLE_ROOMS.some((r) => withMales.has(r))) {
      tripleFull = true;
  ...
  return {
    availableRooms: [...new Set(available)],
...
/** Write the allocated room onto the booking rows, once it has one. */
export async function setBookingRoom(ids, room) {
  const list = (Array.isArray(ids) ? ids : [ids]).filter(Boolean);
  if (USE_SQL_FILE || !list.length || !room) return { ok: true, dryRun: true };
  const pool = await getPool();
  await pool.query(
    `UPDATE bookings SET room_no=? WHERE b_id IN (${list.map(() => "?").join(",")})`,
    [room, ...list]
  );
  return { ok: true };
}
...
/**
 * The room the school allocated, read back from the booking.
 *
 * The student no longer picks a room and the panel no longer sends one, so the
 * payment routes cannot take it from the request body — they read it from the
 * row that step 2 already wrote. The ashram's own notification needs it; the
 * student's confirmation deliberately does not carry it.
 */
export async function getBookingRoom(ids) {
  const list = (Array.isArray(ids) ? ids : [ids]).filter(Boolean);
  if (USE_SQL_FILE || !list.length) return "";
  try {
    const pool = await getPool();
    const [rows] = await pool.query(
      `SELECT room_no FROM bookings WHERE b_id IN (${list.map(() => "?").join(",")}) LIMIT 1`,
      list
    );
    const room = rows?.[0]?.room_no || "";
    return room && room !== "N/A" ? room : "";
  } catch {
    // A notification without the room beats a failed payment confirmation.
    return "";
  }
}
...
export async function sendStudentConfirmation({ student, course, batch, acco, fees }) {
  const user = process.env.SMTP_USER || "info@adhiroha.com";
  const pass = process.env.SMTP_PASS;
  ...
  const html = `
    <div style="font-family:sans-serif;color:#2c2723;max-width:560px">
      ...
      <table style="font-size:14px;border-collapse:collapse;margin:18px 0">
        ...
        <!-- Sharing type only. The room number is allocated by the school and
             is not something the student is told in advance — it can still
             change before they arrive, and the same line in the ashram's own
             notification does carry it. -->
        <tr><td style="padding:6px 14px 6px 0;color:#8a8078"><b>Accommodation</b></td><td>${esc(acco)}</td></tr>
