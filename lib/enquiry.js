 ...
 */
/**
 * Pages that take enquiries instead of bookings.
 *
 * Hatha, Ashtanga and Pranayama are detected at runtime by the ".bc-pill.is-soon"
 * marker their pages carry, because those courses are not running as scheduled
 * batches. The Sadhana Immersion Programme *is* running, on fixed 1st and 15th
 * batches, but is arranged with the school directly rather than through the
 * admission panel, so it is listed here explicitly instead of being marked
 * "upcoming", which would be untrue.
 */
export const ENQUIRY_ONLY_PATHS = [
