...
//
// These are deliberately NOT emitted as schema.org aggregateRating. Google's
// rich-result policy requires ratings for an organisation or local business to
// come from users rather than be compiled by the site itself, and a flat 5.0
// with a round review count and no accompanying Review nodes is exactly the
// self-serving pattern that earns a structured-data manual action. To show
// stars in search again, surface real reviews — a Google Business Profile
// widget that emits its own markup, or Review nodes carrying a genuine author,
// datePublished and reviewBody.
...
export const COURSE_FACTS = {
  ...
  "/yoga-retreat-in-rishikesh/": {
    image: "/gallery/retreat/retreat-008.webp",
    startsOnDays: [24],
    // No certificate — the retreat page is explicit that it does not award one.
...
export function personSchema({ name, subject, image, bio, years }) {
  const p = {
    "@type": "Person",
    name,
