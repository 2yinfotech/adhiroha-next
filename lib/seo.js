...
export function organizationSchema() {
  return {
    ...
    // The school's own Google rating, displayed on the homepage and on each
    // course page with the same figures, so the markup and the page agree.
...
export function courseSchema({
  ...
  // No aggregateRating here. The 383 Google reviews are of the school, not of
  // one course, and putting the same figure on all three Course nodes would
  // claim each course had 383 ratings of its own. It sits on the Organization
  // node instead, which is what it actually describes. Per-course ratings can
  // move back here once the school collects them per course.
  return schema;
}
