...
//   (REG_* is accepted too, see lib/db-config.js)
...
async function getPool() {
  ...
  _pool = await openPool("blog", siteDbConfig());
  return _pool;
}
...
function loadSqlDump() {
  ...
  const mainSql = fs.readFileSync(path.join(process.cwd(), "u511577297_adhiroha_clean.sql"), "utf8");
  // Prefer the dedicated, kept-up-to-date articles.sql for article content when
  // it exists; fall back to the full dump. FAQs live only in the full dump.
  const articlesFile = path.join(process.cwd(), "articles.sql");
  const articlesSql = fs.existsSync(articlesFile) ? fs.readFileSync(articlesFile, "utf8") : mainSql;
  _sqlCache = {
    articles: parseInsert(articlesSql, "articles", ARTICLE_COLS),
    faqs: parseInsert(mainSql, "article_faqs", FAQ_COLS),
  };
  return _sqlCache;
}
...
// top-level route folders under app/(en)/(main). Used to classify links in bodies.
export async function getInternalOk() {
  const set = await getAllSlugs();
  try {
    const fs = require("fs");
    const path = require("path");
    for (const d of fs.readdirSync(path.join(process.cwd(), "app", "(en)", "(main)"), { withFileTypes: true })) {
