...
import { registrationDbConfig, openPool } from "@/lib/db-config";
 ...
 *
 * Configured through REG_DATABASE_HOST and REG_DB_PORT / USER / PASSWORD / NAME,
 * falling back to the site's DB_* only so local development works without a
 * second set of credentials. On the server these must be set, or this form would
 * read the wrong bookings.
 */
let _pool = null;
async function getPool() {
  if (_pool) return _pool;
  _pool = await openPool("registration", registrationDbConfig());
  return _pool;
}

/** Which database this form is actually pointed at, for the health check. */
export const registrationDbName = () => registrationDbConfig().database || "(unset)";
...
export async function getBooking(bookingId) {
  ...
  try {
    ...
    console.error(
      ...
      `${registrationDbConfig().host} is unreachable: ` +
      `${e.code || ""} ${e.message}`,
    );
